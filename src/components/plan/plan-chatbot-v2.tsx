...
import { usePlanChatbotSessionV2 } from "@/hooks/usePlanChatbotSession-v2";

export default function PlanChatbotV2({
  onPlanUpdate,
}: {
  onPlanUpdate?: () => void;
}) {
    ...
    if (messages.length > 0) {
      ...
      if (
        ...
        if (isSuccessMessage && lastSuccessMessageId !== messageId) {
          console.log(
            "[PlanChatbotV2] Detected successful plan update, triggering refresh",
          );
          console.log("[PlanChatbotV2] Message content:", lastMessage.content);
